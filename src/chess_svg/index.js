...
const cords = {
  ...
  4: '<path d="M11.016 18.035L7.03 24.262h3.985zm-.414-1.375h1.984v7.602h1.664v1.312h-1.664v2.75h-1.57v-2.75H5.75v-1.523z"/>',
  ...
  7: '<path d="M6.25 16.66h7.5v.672L9.516 28.324H7.867l3.985-10.336H6.25z"/>',
  ...
  f: '<path d="M25.285 2.258v1.195H23.91q-.773 0-1.078.313-.297.312-.297 1.125v.773h2.367v1.117h-2.367v7.633H21.09V6.781h-1.375V5.664h1.375v-.61q0-1.46.68-2.124.68-.672 2.156-.672z"/>',
...
const renderSVG = (board, whiteBottom) => {
  ...
  board.squares.map((square) => {
    ...
    svgElements.push(`<rect x="${x}" y="${y}" width="45" height="45" class="square dark a1" stroke="none" fill="${color}"/>`)
...
app.get('/board.jpeg', (req, res) => {
  let { fen, rotate } = req.query
  if (!fen) {
  ...
  fabric.loadSVGFromString(svg, (objects, info) => {
    const ctx = canvas.getContext('2d')
    const scaleX = info.width ? (size / info.width) : 1
    const scaleY = info.height ? (size / info.height) : 1
    ctx.scale(scaleX, scaleY)
    const obj = fabric.util.groupSVGElements(objects, info)
    canvas.add(obj)
    canvas.renderAll()
